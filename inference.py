...
def prepare_dataset(settings_dataset, settings_features, config, inference_params):
    ...
    if os.path.exists(Path("/src/aux_files/data/data_splits_features_OLD")):
        shutil.rmtree(Path("/src/aux_files/data/data_splits_features_OLD"))
...
def _do_inference(model: Module,
    ...
    caps = caps["caption_predicted"].iloc[-1]
    logger_main.info("caps: {}", caps)
    return caps
